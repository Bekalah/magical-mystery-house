...
func _ready():
	...
	if player:
		...
		# Set the character in GameManager after it's loaded
		GameManager.current_character = player
		
		# Emit signal again now that character is actually loaded
		GameManager.character_changed.emit(player)
