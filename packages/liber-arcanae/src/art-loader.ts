...
export interface CardArtAsset {
  ...
  backImage?: string; // Seal of Abyssiae
  thumbnail?: string;
...
/**
 * ⚗️ LiberArcanaeArtLoader - The Crucible
 * 
...
export class LiberArcanaeArtLoader {
  private artAssets: Map<string, CardArtAsset> = new Map();
  private artBasePath: string;
  private sealImage?: string;

  constructor(artBasePath: string = '/data/liber-arcanae/art') {
   ...
   */
  private async loadSeal(): Promise<void> {
    try {
...
// console.error('Error loading seal:', error);
   ...
   */
  private generateSealSVG(): string {
   ...
   */
  async loadCardArt(cardId: string, config?: ArtGeneratorConfig): Promise<CardArtAsset> {
    ...
    let artAsset: CardArtAsset | null = null;
   ...
   */
  private async generateCardArt(
    cardId: string,
    config?: ArtGeneratorConfig
  ): Promise<CardArtAsset> {
   ...
   */
  private generatePlaceholderSVG(cardId: string): string {
    return `data:image/svg+xml;base64,${btoa(`
   ...
   */
  async loadAllMajorArcana(): Promise<Map<string, CardArtAsset>> {
    const majorArcana = Array.from({ length: 22 }, (_, i) => `major_${i}`);
    const assets = new Map<string, CardArtAsset>();
