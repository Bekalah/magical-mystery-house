...
function drawVesica(ctx, w, h, color, NUM) {
  /* Vesica grid: calm outlines, no fill. */
  ctx.save();
...
function drawTree(ctx, w, h, color, NUM) {
  /* Tree-of-Life: 10 nodes with 22 paths. */
  ctx.save();
  const nodes = [
...
function drawFibonacci(ctx, w, h, color, NUM) {
  /* Log spiral polyline; static for ND safety. */
  ctx.save();
  ctx.strokeStyle = color;
  ctx.lineWidth = 2;
...
function drawHelix(ctx, w, h, color, NUM) {
  /* Double helix lattice: two strands with rungs, no motion. */
  ctx.save();
  ...
  for (let i = 0; i <= NUM.THIRTYTHREE; i++) {
    ...
    const y1 = h/2 + Math.sin((i/NUM.THIRTYTHREE) * Math.PI * turns) * amp;
    const y2 = h/2 + Math.sin((i/NUM.THIRTYTHREE) * Math.PI * turns + Math.PI) * amp;
    ctx.beginPath();
    ctx.moveTo(x, y1);
    ctx.lineTo(x, y2);
    ctx.stroke();
