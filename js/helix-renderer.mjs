...
function drawVesica(ctx, w, h, color, NUM) {
  /* Vesica field: overlapping circles create a calm grid.
     ND-safe: thin strokes, low repetition. */
  ctx.save();
...
function drawTree(ctx, w, h, pathColor, nodeColor, NUM) {
  /* Tree-of-Life: 10 sephirot and 22 connective paths.
  ...
  const paths = [
    [0,1],[0,2],[1,2],[1,3],[2,4],[3,5],[4,5],[3,6],[4,7],
    [5,6],[5,7],[6,8],[7,8],[8,9],[1,4],[2,3],[1,5],
    [2,6],[3,8],[4,8],[5,9],[6,9]
  ]; // NUM.TWENTYTWO
  ...
  const nodeR = NUM.ELEVEN / 2;
  ctx.fillStyle = nodeColor;
  nodes.forEach(([x,y]) => {
    ctx.beginPath();
    ctx.arc(x*w, y*h, nodeR, 0, Math.PI*2);
    ctx.fill();
  });
  ctx.restore();
}
...
function drawFibonacci(ctx, w, h, color, NUM) {
  /* Fibonacci spiral approximated by polyline.
     ND-safe: single static stroke. */
  ctx.save();
  ctx.strokeStyle = color;
  ctx.lineWidth = 2;
...
// Layer 4 ---------------------------------------------------------------
function drawLattice(ctx, w, h, strandColor, rungColor, NUM) {
  /* Double-helix lattice: two sine-wave strands with cross rungs.
     ND-safe: static, readable contrast, no flashing. */
  ctx.save();
