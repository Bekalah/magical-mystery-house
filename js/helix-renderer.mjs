...
export function renderHelix(ctx, opts) {
  ...
  ctx.save();
  ctx.fillStyle = palette.bg; // calm backdrop
  ctx.fillRect(0, 0, width, height);

  const [vesicaColor, pathColor, nodeColor, fibColor, helixColor, rungColor] = palette.layers;
  drawVesica(ctx, width, height, vesicaColor, NUM);
  drawTree(ctx, width, height, pathColor, nodeColor, NUM);
  drawFibonacci(ctx, width, height, fibColor, NUM);
  drawHelix(ctx, width, height, helixColor, rungColor, NUM);
  ctx.restore();
}

// Layer 1 ---------------------------------------------------------------
function drawVesica(ctx, w, h, color, NUM) {
  /* Vesica field: low-density circle lattice.
     ND-safe: thin lines, no motion. */
  ctx.save();
...
function drawTree(ctx, w, h, pathColor, nodeColor, NUM) {
  /* Tree-of-Life: 10 nodes, 22 paths.
     ND-safe: steady lines, solid nodes. */
  ctx.save();
  const nodes = [
...
function drawFibonacci(ctx, w, h, color, NUM) {
  /* Fibonacci spiral approximated with 144 points.
     ND-safe: static path, gentle stroke. */
  ctx.save();
  ctx.strokeStyle = color;
  ctx.lineWidth = 2;
...
function drawHelix(ctx, w, h, strandColor, rungColor, NUM) {
  /* Double helix: two phase-shifted sine strands with 33 rungs.
     ND-safe: static lattice, readable contrast. */
  ctx.save();
