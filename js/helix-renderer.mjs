...
function drawVesica(ctx, w, h, color, NUM) {
  /* Vesica field: calm outline grid. ND-safe via thin lines and low density. */
  ctx.save();
  ctx.strokeStyle = color;
  ctx.lineWidth = 1;
  const r = Math.min(w, h) / NUM.THREE; // sacred triad
...
function drawTree(ctx, w, h, pathColor, nodeColor, NUM) {
  /* Tree-of-Life scaffold; ND-safe with gentle lines and solid nodes. */
  ctx.save();
  const nodes = [
...
function drawFibonacci(ctx, w, h, color, NUM) {
  /* Fibonacci logarithmic spiral approximated by a static polyline. */
  ctx.save();
  ctx.strokeStyle = color;
  ctx.lineWidth = 2;
