...
const WORKFLOWS = {
  'pre-commit': {
    ...
    steps: [
      { name: 'Quality Check', cmd: 'pnpm run quality:enhanced' },
      { name: 'Consistency Check', cmd: 'pnpm run check:consistency' },
      { name: 'Build', cmd: 'pnpm run build' },
      { name: 'Tests', cmd: 'pnpm run test:run' }
    ]
  },
  'post-update': {
    ...
    steps: [
      { name: 'Auto-Update', cmd: 'pnpm run auto:update' },
      { name: 'Recovery', cmd: 'pnpm run recover:auto' },
      { name: 'Health Check', cmd: 'pnpm run health:monitor:once' },
      { name: 'Documentation', cmd: 'pnpm run docs:auto' }
    ]
  },
  'daily': {
    ...
    steps: [
      { name: 'Quick Maintenance', cmd: 'pnpm run maintain:quick' },
      { name: 'Health Check', cmd: 'pnpm run health:monitor:once' },
      { name: 'Backup', cmd: 'pnpm run backup:create' }
    ]
  },
  'weekly': {
    ...
    steps: [
      { name: 'Full Maintenance', cmd: 'pnpm run maintain:full' },
      { name: 'Performance Optimization', cmd: 'pnpm run optimize:performance' },
      { name: 'Integration Tests', cmd: 'pnpm run test:integration' },
      { name: 'System Integration', cmd: 'pnpm run integrate:systems' }
    ]
  },
  'full-check': {
    ...
    steps: [
      ...
      { name: 'Quality Check', cmd: 'pnpm run quality:enhanced' },
      { name: 'Integration Tests', cmd: 'pnpm run test:integration' },
      { name: 'Performance Check', cmd: 'pnpm run optimize:performance' }
    ]
  }
};
...
function listWorkflows() {
  ...
  for (const [key, workflow] of Object.entries(WORKFLOWS)) {
    ...
    UserFeedback.command(`pnpm run workflow:${key}`, `Run ${workflow.name}`);
  }
}
