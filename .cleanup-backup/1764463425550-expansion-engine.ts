...
class ExpansionEngine {
  ...
  private async fixIssue(opportunity: ImprovementOpportunity): Promise<ImprovementResult> {
    ...
    if (opportunity.description.toLowerCase().includes('security') || 
        opportunity.description.toLowerCase().includes('validation') || 
        opportunity.description.toLowerCase().includes('sanitization') ||
        opportunity.description.toLowerCase().includes('package.json')) {
      try {
        // @ts-expect-error - TypeScript limitation: dynamic ESM imports don't resolve .d.ts declarations
        // Declaration exists in packages/trinity-v1-1-core/apply-security-fixes-monorepo.d.ts
