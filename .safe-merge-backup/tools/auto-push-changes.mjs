...
class AutoPushChanges {
  ...
  async pushAllChanges() {
    logger.info('🚀 Pushing all changes to remote repositories...');
    
    const results = [];
    
    for (const repoPath of this.repos) {
      if (!fs.existsSync(repoPath)) {
        logger.warn(`   ⚠️  Repository not found: ${repoPath}`);
        continue;
      }
      ...
      try {
        ...
        logger.error(`   ❌ Error pushing ${path.basename(repoPath)}: ${error.message}`);
        results.push({
    ...
    logger.info(`\n✅ Pushed changes to ${successful}/${results.length} repositories`);
  ...
  async pushRepo(repoPath) {
    ...
    if (hasStaged) {
      ...
      logger.info(`   ✅ Committed changes in ${repoName}`);
    }
    ...
    for (const remote of remotes) {
      try {
        ...
        logger.warn(`   ⚠️  Could not push to ${remote}: ${error.message}`);
      }
    }
  ...
  async monitorRemoteWork() {
    logger.info('👀 Monitoring remote work across repositories...');
    ...
    for (const repoPath of this.repos) {
      ...
      try {
        ...
        logger.warn(`   ⚠️  Could not check ${path.basename(repoPath)}: ${error.message}`);
      }
    }
    
    logger.info(`\n📊 Remote status for ${status.length} repositories`);
