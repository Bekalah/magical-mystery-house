...
export class FractalSoundSynthesizer {
  ...
  public static synthesizeSample(pattern: FractalSoundPattern, time: number): number {
    let sample = 0;
    ...
    sample *= (1.0 + Math.sin(2 * Math.PI * pattern.phi_modulation * time) * 0.1);

    return Math.max(-1, Math.min(1, sample)); // Clamp to [-1, 1]
  }
  ...
  public static generateAudioBuffer(pattern: FractalSoundPattern): Float32Array {
    ...
    for (let i = 0; i < bufferLength; i++) {
      ...
      buffer[i] = FractalSoundSynthesizer.synthesizeSample(pattern, time);
    }
