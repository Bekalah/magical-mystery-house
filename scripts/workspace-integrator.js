...
export class WorkspaceIntegrator {
    ...
    scanWorkspaces() {
        try {
            ...
            const workspacePaths = [
                { path: path.join(this.baseDir, 'cathedral-real'), name: 'cathedral-real', type: 'real', priority: 'high' },
                { path: path.join(this.baseDir, 'cathedral-fixed-clean'), name: 'cathedral-fixed-clean', type: 'fixed', priority: 'high' },
                { path: path.join(this.baseDir, 'cathedral-master-deployment'), name: 'cathedral-master-deployment', type: 'deployment', priority: 'high' },
                { path: path.join(this.baseDir, 'cathedral'), name: 'cathedral', type: 'other', priority: 'medium' }
            ];
            let found = 0;
    ...
    determineUpdateNeeded(wsPath, _name) {
        try {
    ...
    determinePriority(_name, type, needsUpdate) {
        if (!needsUpdate) {
            return 'low';
        }
    ...
    mergePackageJson(masterPath, targetPath) {
        try {
    ...
    buildMissingComponents(workspacePath) {
        let built = 0;
    ...
    validateIntegration(workspaceName) {
        const errors = [];
        const warnings = [];
